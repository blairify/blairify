...
class InterviewQuestionSeeder {
  ...
  extractExamples(answer) {
    ...
    if (examplePatterns) {
      examples.push(
        ...examplePatterns.map((ex) => ex.replace(/example[s]?:?\s*/i, "")),
      );
    }
  ...
  saveToJSON() {
    const outputPath = path.join(
      __dirname,
      "../src/data/questions/interview-questions.json",
    );
  ...
  generateSummary() {
    ...
    this.questions.forEach((q) => {
      // By language
      summary.byLanguage[q.language || "unknown"] =
        (summary.byLanguage[q.language || "unknown"] || 0) + 1;

      // By difficulty
      summary.byDifficulty[q.difficulty]++;

      // By category
      summary.byCategory[q.category] =
        (summary.byCategory[q.category] || 0) + 1;
    });
  ...
  run() {
    ...
    console.log(
      "1. Review the generated questions in src/data/questions/interview-questions.json",
    );
