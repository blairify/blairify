...
export function MessageInput({
  ...
  const MAX_CHARS = 2000; // Limit to prevent token overload
  const charCount = value.length;
  const hasContent = value.trim().length > 0;
  const isNearLimit = charCount > MAX_CHARS * 0.8; // Warning at 80%
        ...
        <div className="flex items-end gap-2 p-3">
          ...
          <div className="flex-1 relative">
            <Textarea
              ...
              className="min-h-[40px] max-h-[160px] resize-none border-0 bg-transparent p-0 text-base placeholder:text-muted-foreground focus-visible:ring-0 focus-visible:ring-offset-0 overflow-y-auto"
              onKeyDown={(e) => {
                if (
                  e.key === "Enter" &&
                  !e.shiftKey &&
