const LANGUAGE_REQUEST_PATTERNS: RegExp[] = [
  // === English Variants ===
...
const PROFANITY_PATTERNS: RegExp[] = [
  ...
  // === POLISH (expanded) ===
  /\b(kurwa|kurwy|kurwa mać|kurwica|kurwią|spierdalaj|spierdalać|pierdol|pierdolę|pierdolony|pierdolona|pierdoleni|pierdolcie|pierdolnę|jeb|jebie|jebiecie|jebane|jebany|jebana|jebani|chuj|kutas|fiut|cipa|pizda|skurwysyn|suka|dziwka|debil|idiota|imbecyl|ćwok|kretyn|menda|gówno|srać|zasrany)\b/i,
...
export function detectDisallowedTopic(message: string) {
  ...
  const containsDisallowedTopic = DISALLOWED_TOPIC_PATTERNS.some((pattern) =>
    pattern.test(normalized),
  );
...
function anyToken(tokens: string[]): RegExp {
  const parts = tokens
