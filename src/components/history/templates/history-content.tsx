    ...
    <main className="flex-1 overflow-y-auto p-6 pb-24 animate-in fade-in duration-500">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
          ...
          {Array.from({ length: Math.max(0, 10 - sessions.length) }).map(
            (_, index) => (
              <div
                key={`skeleton-${index}`}
                className="aspect-square w-full flex flex-col items-center justify-center gap-4 p-4 bg-background border border-border/10 rounded-2xl"
              >
                <div className="relative size-[90px]">
                  <div className="size-full rounded-full border-10 border-muted/60" />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-12 h-12 bg-muted/60 rounded-full" />
                  </div>
                </div>
                <div className="flex flex-col gap-1 items-center min-w-0 w-full">
                  <div className="w-24 h-4 bg-muted/60 rounded" />
                  <div className="w-16 h-3 bg-muted/55 rounded" />
                </div>
              </div>
            ),
          )}
