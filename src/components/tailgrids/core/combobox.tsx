...
export function ComboboxInputWrapper({
  ...
  return (
    <div
      className={cn(
        "bg-input-background relative flex w-full min-w-sm h-10 items-center justify-between rounded-lg border border-base-300 hover:border-base-400 focus-within:border-input-primary-focus-border focus-within:ring-3 focus-within:ring-gray-500",
        context?.isInvalid &&
...
export function ComboboxInput({ className, ...props }: AriaInputProps) {
  return (
    <AriaInput
      className={cn(
        "w-full h-10 bg-transparent pl-4 pr-8 py-2 text-text-50 font-medium placeholder:text-input-placeholder-text outline-none disabled:cursor-not-allowed disabled:text-input-disabled-text",
        className,
      )}
      {...props}
    />
...
export function ComboboxContent({
  ...
  return (
    <Popover
      className={cn(
        "bg-card w-(--trigger-width) overflow-auto rounded-xl border border-base-100 shadow-md",
